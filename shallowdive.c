...
uint32_t bitcrushing_value=4096^256; // 8 bit by default.
...
int main(int argc, char **argv)
{
	...
	if (PUSH2_val==0)
		{ bcm2835_delay(100); //100ms delay for buttons debouncing
         if (bitcrushing_value<4096) bitcrushing_value = bitcrushing_value<<1;
		}
		else if (PUSH1_val==0) 
			{bcm2835_delay(100); //100ms delay for buttons debouncing.
			if (bitcrushing_value>2) {
				bitcrushing_value = (bitcrushing_value>>1)+1;
			}
	}

	//**** BITCRUSHING EFFECT ***///
	//The depth is reduced by the bitwise & operator
	// this reduces the finess, example: 1101 & 1100 = 1100
